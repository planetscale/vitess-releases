...
sudo apt-get -y install make automake libtool libssl-dev g++ git pkg-config bison curl unzip zip

DIR=$(pwd)

INSTALL_GO=0
if [ -f /usr/local/go/bin/go ]; then
    /usr/local/go/bin/go version | grep -q go1.11.4
    if [ $? -ne 0 ]; then
	INSTALL_GO=1
    fi
else
...
for d in $(echo bin lib dist config pkg vthook); do
    mkdir -p ${RELEASE_DIR}/$d
    echo cp -Lrpf $HOME/go/$d/. ${RELEASE_DIR}/$d
    cp -Lrpf $HOME/go/$d/. ${RELEASE_DIR}/$d
done

for d in $(echo chromedriver grpc MYSQL_FLAVOR py-mock-1.0.1 selenium vt-protoc-3.6.1); do
